...
ldflags=-X=github.com/filecoin-project/lotus/build.CurrentCommit=+git.$(subst -,.,$(shell git describe --always --match=NeVeRmAtCh --dirty 2>/dev/null || git rev-parse --short HEAD 2>/dev/null)) -X=github.com/filecoin-project/lotus/build.Feature=+zt0.3
ifneq ($(strip $(LDFLAGS)),)
